...
def run_ftest(ws_path, log_name, inputpath_name, process, isclean):
    ...
    if isclean:
        ...
        rm_p = subprocess.call(command, shell=True, stdout=subprocess.PIPE)
    #copy config_yh2018.py
    command = "cp config_yh2018.py " + "config_" +process+ ws_path + ".py"
    run_p = subprocess.call(command, shell=True, stdout=subprocess.PIPE)
    logging.info("copy config_yh2018.py \n command :{0}".format(command))
    time.sleep(2)
    # sed config_*.py
...
for mass in mass_list:
    for cat in cat_list:
        ...
        # ------------------------------------- background fit -------------------------------------
        input_path_name = "/eos/cms/store/group/phys_b2g/shsong/YH_flashggws/cat34/2018/" + mass + "/"
        ...
        #     log_file_name_signal = mass +"_yhwwgg_MC_2018_"+cat+final_state+".log"
        #     ws_path_signal = mass +"_2018_"+cat+"_"+final_state
        #     output_root_name_signal = "output_Signal"+mass + cat + "_M125_2018_13TeV_amcatnloFXFX_pythia8_ggyh" + final_state + ".root"
        ...
        #     run_signal_plot(cats=cat, exts="2018_" + mass +"_2018_" + cat +"_"+ final_state, outputExt="packaged_" + mass +"_2018_" + cat +"_"+final_state, log_packaged_name = log_path + "packaged_" + mass + cat+".log", ws_path=ws_path_signal, inputpath_name =input_path_name, log_plotter_name = log_path + "plotter_" + mass + cat+".log", cp_name="CMS-HGG_sigfit_packaged_"+cat+"_2018.root", process = 'ws_ggyh'+ final_state)
        #     run_yields(ws_sig_path=ws_path_signal, log_name = log_path + mass +"_2018_" + cat +"_yields.log" , inputpath_name = input_path_name, ws_bkg_path = ws_data_path, process='ws_ggyh'+ final_state )
        #     run_makeDatacard(ws_sig_path=ws_path_signal, log_name = log_path + mass + cat + "_makeDatacard.log" , output_card_name = "Datacard_" + mass +"_2018_" +final_state +"_"+ cat, channel=mass,process='ggyh'+ final_state) 
