...
def run_Tree2WS_sig(inputpath_name, inputfile_name, log_file_name, ws_path, output_sig_root_name, process):
    ...
    command = " python trees2ws.py --inputConfig config_sys16.py --inputTreeFile " + inputpath_name+inputfile_name + " --inputMass 125  --doSystematics --productionMode " + process_prefix + " --year 2016pre  > " + log_file_name+ " 2>&1"
...
def run_ftest(ws_path, log_name, inputpath_name, process, isclean = False):
    ...
    if isclean:
        command = "rm -rf outdir_2016pre_" + ws_path
...
def run_signalfit(ws_path, log_name, inputpath_name, process):
    ...
    command = "cp outdir_2016pre_" + ws_path + "/signalFit/output/*.root" + " " + inputpath_name +  process + "_"+ws_path
    cp_p = subprocess.call(command, shell=True, stdout=subprocess.PIPE)
...
def run_yields(ws_sig_path,ws_bkg_path, inputpath_name,log_name, process):
    ...
    # command = "python RunYields.py --inputWSDirMap 2016pre=" +inputpath_name+ process + "_" + ws_sig_path+" --sigModelWSDir " + inputpath_name+ process + "_" + ws_sig_path+" --bkgModelWSDir " + inputpath_name+  ws_bkg_path + " --cats auto --doSystematics --procs auto --batch local --ext "+ ws_sig_path+ ">" + log_name+ " 2>&1"
        command = "python RunYields_16boosted_bbgg.py --inputWSDirMap 2016pre=" +inputpath_name+ process + "_" + ws_sig_path+" --sigModelWSDir " + inputpath_name+ process + "_" + ws_sig_path+" --bkgModelWSDir " + inputpath_name+  ws_bkg_path + " --cats auto --doSystematics --procs "+process.split("ws_")[-1]+" --batch local --ext "+ ws_sig_path+ ">" + log_name+ " 2>&1"
    else:
        command = "python RunYields_16boosted.py --inputWSDirMap 2016pre=" +inputpath_name+ process + "_" + ws_sig_path+" --sigModelWSDir " + inputpath_name+ process + "_" + ws_sig_path+" --bkgModelWSDir " + inputpath_name+  ws_bkg_path + " --cats auto --doSystematics --procs "+process.split("ws_")[-1]+" --batch local --ext "+ ws_sig_path+ ">" + log_name+ " 2>&1"
...
def run_makeDatacard(ws_sig_path,output_card_name,log_name,channel,process):
    ...
    # command = "python makeDatacard.py --years 2016pre --doSystematics --prune --ext " + ws_sig_path + " --output "+ output_card_name+  ">" + log_name+ " 2>&1"
        command = "python makeDatacard_16boosted.py --years 2016pre --doSystematics --prune --ext " + ws_sig_path + " --output "+ output_card_name+  ">" + log_name+ " 2>&1"
    elif "bbgg" in output_card_name:
        command = "python makeDatacard_16boosted_bbgg.py --years 2016pre --doSystematics --prune --ext " + ws_sig_path + " --output "+ output_card_name+  ">" + log_name+ " 2>&1"
    elif "zzgg" in output_card_name:
        command = "python makeDatacard_16boosted_zzgg.py --years 2016pre --doSystematics --prune --ext " + ws_sig_path + " --output "+ output_card_name+  ">" + log_name+ " 2>&1"
    elif "ttgg" in output_card_name:
        command = "python makeDatacard_16boosted_ttgg.py --years 2016pre --doSystematics --prune --ext " + ws_sig_path + " --output "+ output_card_name+  ">" + log_name+ " 2>&1"
    else:
        command = "python makeDatacard_16boosted.py --years 2016pre --doSystematics --prune --ext " + ws_sig_path + " --output "+ output_card_name+  ">" + log_name+ " 2>&1"
...
for mass in mass_list:
    for cat in cat_list:
        ...
        # ------------------------------------- background fit -------------------------------------
        input_path_name = "/eos/cms/store/group/phys_b2g/shsong/flashggws/cat12_graviton/2016pre/" + mass + "/"
        ...
        ext_name = "ws_2016pre_" + cat + "_"+ mass
        ...
        for final_state in final_state_list:        
            # # # ------------------------------------- signal fit -------------------------------------
            input_file_name_signal = mass +"_2016pre_"+final_state+"_"+cat+".root"
            ...
            # run_makeDatacard(ws_sig_path=ws_path_signal, log_name = log_path + mass + cat + "_makeDatacard.log" , output_card_name = "Datacard_" + mass +"_2016pre_" +final_state +"_"+ cat, channel=mass,process='gghh'+ final_state) 
