...
def run_ftest(ws_path, log_name, inputpath_name, process, isclean):
    ...
    command = "cp config_nosys_yh.py " + "config_" +process+ ws_path + ".py"
    run_p = subprocess.call(command, shell=True, stdout=subprocess.PIPE)
    # logging.info("copy config_nosys.py \n command :{0}".format(command))
    logging.info("copy config_nosys_yh.py \n command :{0}".format(command))
    time.sleep(2)
...
def run_yields(ws_sig_path,ws_bkg_path, inputpath_name,log_name, process):
    ...
    # run Runyields
    command = "python RunYields_17resolved.py --inputWSDirMap 2017=" +inputpath_name+ process + "_" + ws_sig_path+" --sigModelWSDir " + inputpath_name+ process + "_" + ws_sig_path+" --bkgModelWSDir " + inputpath_name+  ws_bkg_path + " --cats auto --procs "+process.split("ws_")[-1]+" --batch local --ext "+ ws_sig_path+ ">" + log_name+ " 2>&1"
...
def run_makeDatacard(ws_sig_path,output_card_name,log_name,channel,process):
    ...
    # run RunmakeDatacard
    if "wwgg" in process:
        command = "python makeDatacard_17resolved.py --years 2017 --prune --ext " + ws_sig_path + " --output "+ output_card_name+  ">" + log_name+ " 2>&1"
    else:
        command = "python makeDatacard_17resolved_singlehiggs.py --years 2017 --prune --ext " + ws_sig_path + " --output "+ output_card_name+  ">" + log_name+ " 2>&1"
...
for mass in mass_list:
    for cat in cat_list:
        ...
        # ------------------------------------- background fit -------------------------------------
        input_path_name = "/eos/cms/store/group/phys_b2g/shsong/YH_flashggws/cat34/2017/" + mass + "/"
