...
if opt.cats in ['all','wall']:
  ...
  for cat in opt.cats.split(","):
    for year in opt.years.split(","):
      ...
      f = "%s/outdir_%s/CMS-HGG_sigfit_%s_%s_%s.root"%(swd__,ext_name,ext_name,cat,year) #attention: only have 2017 year result with the name *_2017 so we add the _2017 in the str here
      # f = "%s/outdir_%s/CMS-HGG_sigfit_%s_%s.root"%(swd__,opt.ext,opt.ext,cat)
      f.replace("2018",year)
      f.replace("2017",year)
      f.replace("2016pre",year)
      f.replace("2016post",year)
      f = f.replace("_"+year,"")
      inputFiles[cat] = f
      if citr == 0:
        w = ROOT.TFile(f).Get("wsig_13TeV")
