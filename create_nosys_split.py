...
def run_Tree2WS_sig(inputpath_name, inputfile_name, log_file_name, ws_path, output_sig_root_name, process):
    ...
    command = " python trees2ws.py --inputConfig config_simple.py --inputTreeFile " + inputpath_name+inputfile_name + " --inputMass 125  --productionMode " + process_prefix + " --year 2016pre  > " + log_file_name+ " 2>&1"
...
def run_yields(ws_sig_path,ws_bkg_path, inputpath_name,log_name, process):
    ...
    # run Runyields
    command = "python RunYields.py --inputWSDirMap 2016pre=" +inputpath_name+ process + "_" + ws_sig_path+" --sigModelWSDir " + inputpath_name+ process + "_" + ws_sig_path+" --bkgModelWSDir " + inputpath_name+  ws_bkg_path + " --cats auto --procs auto --batch local --ext "+ ws_sig_path+ ">" + log_name+ " 2>&1"
...
def run_makeDatacard(ws_sig_path,output_card_name,log_name,channel,process):
    ...
    command = "python makeDatacard.py --years 2016pre --prune --ext " + ws_sig_path + " --output "+ output_card_name+  ">" + log_name+ " 2>&1"
...
# mass_list=['MX250_MH125','MX1000_MH125']
mass_list=['MX500_MH125']
final_state = ""
# cat_list = ['combineFHSL_cat34FHlowpurity','combineFHSL_cat34FHhighpurity','combineFHSL_cat34SLlowpurity','combineFHSL_cat34SLhighpurity','bbgg_cat34FHhighpurity','bbgg_cat34FHlowpurity','zzgg_cat34FHhighpurity','zzgg_cat34FHlowpurity','bbgg_cat34SLhighpurity','bbgg_cat34SLlowpurity','zzgg_cat34SLhighpurity','zzgg_cat34SLlowpurity']
cat_list = ['combineFHSL_cat34FHlowpurity','combineFHSL_cat34FHhighpurity','combineFHSL_cat34SLlowpurity','combineFHSL_cat34SLhighpurity','bbgg_cat34FHhighpurity','bbgg_cat34FHlowpurity']
for mass in mass_list:
    for cat in cat_list:
        # ------------------------------------- log path -------------------------------------
        log_path = "/eos/user/s/shsong/hhwwgg_workspace/Final_limit/cat34log/"
        # ------------------------------------- background fit -------------------------------------
        input_path_name = "/afs/cern.ch/user/s/shsong/WWggDNN/bdt/PBDT_HH_FHSL_combine_2016pre/flashgginput/" + mass + "/"
        ...
        input_file_name_signal = "CombineFHSL_"+ mass +"_2016pre_"+cat+".root"
