...
def run_ftest(ws_path, log_name, inputpath_name, process):
    ...
    command4 = 'sed -i "s#dcb_%s_#dcb_%s_' +process + '#g" ' +  "config_" +process+ ws_path + ".py" 
    sed1_p = subprocess.call(command4, shell=True)
    logging.info("sed3 config_*.py \n command :{0}".format(command4))
...
def run_signalfit(ws_path, log_name, inputpath_name, process):
    ...
    command = "cp outdir_dcb_2017_" +process_prefix +ws_path + "/signalFit/output/*.root" + " " + inputpath_name +  process + "_"+ws_path
    cp_p = subprocess.call(command, shell=True, stdout=subprocess.PIPE)
...
def run_signal_plot(outputExt, cats, exts,log_packaged_name,ws_path,inputpath_name,log_plotter_name, cp_name, process):
    ...
    # cp the signalfit output root file to ws
    command = "cp outdir_packaged_" +process.split("ws_")[-1]+ws_path + "/*.root" + " " + inputpath_name +  process + "_"+ ws_path + "/" + cp_name
    cp_p = subprocess.call(command, shell=True, stdout=subprocess.PIPE)
    logging.info("run cp the signalfit output root file to ws \n command :{0}".format(command))
    command = "cp outdir_packaged_" +process.split("ws_")[-1]+ws_path + "/Plots/*.pdf" + " " + inputpath_name +  process + "_"+ ws_path + "/" 
...
def run_yields(ws_sig_path,ws_bkg_path, inputpath_name,log_name, process):
    ...
    # run Runyields
    command = "python RunYields_17resolved.py --inputWSDirMap 2017=" +inputpath_name+ process + "_" + ws_sig_path+" --sigModelWSDir " + inputpath_name+ process + "_" + ws_sig_path+" --bkgModelWSDir " + inputpath_name+  ws_bkg_path + " --cats auto --doSystematics --procs auto --batch local --ext "+ ws_sig_path+ ">" + log_name+ " 2>&1"
...
for mass in mass_list:
    for cat in cat_list:
        ...
        for final_state in final_state_list:   
            ...
            run_signal_plot(cats=cat, exts="dcb_2017_" +"gghh"+final_state+ mass +"_2017_" + cat, outputExt="packaged_" + "gghh"+final_state+mass +"_2017_" + cat, log_packaged_name = log_path + "packaged_" +"gghh" +final_state+mass + cat+".log", ws_path=ws_path_signal, inputpath_name =input_path_name, log_plotter_name = log_path + "plotter_" + mass + cat+".log", cp_name="CMS-HGG_sigfit_packaged_"+cat+"_2017.root", process = 'ws_gghh'+ final_state)
