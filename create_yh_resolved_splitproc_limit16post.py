...
def run_ftest(ws_path, log_name, inputpath_name, process, isclean):
    ...
    if isclean:
        ...
        rm_p = subprocess.call(command, shell=True, stdout=subprocess.PIPE)
    #copy config_yh2016post.py
    command = "cp config_yh2016post.py " + "config_" +process+ ws_path + ".py"
    run_p = subprocess.call(command, shell=True, stdout=subprocess.PIPE)
    logging.info("copy config_yh2016post.py \n command :{0}".format(command))
    time.sleep(2)
    # sed config_*.py
...
def run_yields(ws_sig_path,ws_bkg_path, inputpath_name,log_name, process):
    ...
    if "bbgg" in process:
        ...
        command = "python RunYields_16resolved_bbgg.py --inputWSDirMap 2016post=" +inputpath_name+ process + "_" + ws_sig_path+" --sigModelWSDir " + inputpath_name+ process + "_" + ws_sig_path+" --bkgModelWSDir " + inputpath_name+  ws_bkg_path + " --cats auto --doSystematics --procs "+process.split("ws_")[-1]+" --batch local --ext "+ ws_sig_path+ ">" + log_name+ " 2>&1"
    else:
        command = "python RunYields_16resolved.py --inputWSDirMap 2016post=" +inputpath_name+ process + "_" + ws_sig_path+" --sigModelWSDir " + inputpath_name+ process + "_" + ws_sig_path+" --bkgModelWSDir " + inputpath_name+  ws_bkg_path + " --cats auto --doSystematics --procs "+process.split("ws_")[-1]+" --batch local --ext "+ ws_sig_path+ ">" + log_name+ " 2>&1"
    print(command)
...
def run_makeDatacard(ws_sig_path,output_card_name,log_name,channel,process):
    ...
    if "wwgg" in process:
        ...
        command = "python makeDatacard_16resolved_singlehiggs.py --years 2016post --doSystematics --prune --ext " + ws_sig_path + " --output "+ output_card_name+  ">" + log_name+ " 2>&1"
...
for mass in mass_list:
    for cat in cat_list:
        ...
        # ------------------------------------- background fit -------------------------------------
        input_path_name = "/eos/cms/store/group/phys_b2g/shsong/YH_flashggws/cat34/2016post/" + mass + "/"
        ...
        #     log_file_name_signal = mass +"_yhwwgg_MC_2016post_"+cat+final_state+".log"
        #     ws_path_signal = mass +"_2016post_"+cat+"_"+final_state
        #     output_root_name_signal = "output_Signal"+mass + cat + "_M125_2016post_13TeV_amcatnloFXFX_pythia8_ggyh" + final_state + ".root"
        ...
        #     run_signal_plot(cats=cat, exts="2016post_" + mass +"_2016post_" + cat +"_"+ final_state, outputExt="packaged_" + mass +"_2016post_" + cat +"_"+final_state, log_packaged_name = log_path + "packaged_" + mass + cat+".log", ws_path=ws_path_signal, inputpath_name =input_path_name, log_plotter_name = log_path + "plotter_" + mass + cat+".log", cp_name="CMS-HGG_sigfit_packaged_"+cat+"_2016post.root", process = 'ws_ggyh'+ final_state)
        #     run_yields(ws_sig_path=ws_path_signal, log_name = log_path + mass +"_2016post_" + cat +"_yields.log" , inputpath_name = input_path_name, ws_bkg_path = ws_data_path, process='ws_ggyh'+ final_state )
        #     run_makeDatacard(ws_sig_path=ws_path_signal, log_name = log_path + mass + cat + "_makeDatacard.log" , output_card_name = "Datacard_" + mass +"_2016post_" +final_state +"_"+ cat, channel=mass,process='ggyh'+ final_state) 
