...
def run_ftest(ws_path, inputpath_name, process):
    ...
    # command = "rm -rf outdir_dcb_"+year+"_" + ws_path
...
def run_signalfit(ws_path, inputpath_name, process):
    ...
    command = "python RunSignalScripts.py --inputConfig " +  "config_" +process_prefix+ ws_path + ".py"+ " --mode 'signalFit' --modeOpts '--skipSystematics' " 
...
def run_yields(ws_sig_path,ws_bkg_path, inputpath_name, process):
    os.chdir("./Datacard")
    # run Runyields
    command = "python RunYields_"+yield_cat+".py --inputWSDirMap "+year+"=" +inputpath_name+ process + "_" + ws_sig_path+" --sigModelWSDir " + inputpath_name+ process + "_" + ws_sig_path+" --bkgModelWSDir " + inputpath_name+  ws_bkg_path + " --cats auto --doSystematics --procs auto --batch local --ext "+ ws_sig_path
...
def run_makeDatacard(ws_sig_path,output_card_name,channel,process):
    ...
    # run RunmakeDatacard
    if "wwgg" in process:
        command = "python makeDatacard_17resolved.py --years 2017 --doSystematics --prune --ext " + ws_sig_path + " --output "+ output_card_name
    elif "bbgg" in process:
        command = "python makeDatacard_17resolved_bbgg.py --years 2017 --doSystematics --prune --ext " + ws_sig_path + " --output "+ output_card_name
    elif "zzgg" in process:
        command = "python makeDatacard_17resolved_zzgg.py --years 2017 --doSystematics --prune --ext " + ws_sig_path + " --output "+ output_card_name
    else:
        command = "python makeDatacard_17resolved_singlehiggs.py --years 2017 --doSystematics --prune --ext " + ws_sig_path + " --output "+ output_card_name
...
final_state_list = ["wwgg","bbgg","zzgg"]
...
for cat in cat_list:
    ...
    # ------------------------------------- background fit -------------------------------------
        ...
        print("output_data_root_name: ", output_data_root_name)
        print("ws_data_path: ", ws_data_path)
        ...
        # # ------------------------------------- signal fit -------------------------------------
        input_file_name_signal = mass +"_"+year+"_"+cat+".root"
        ...
        run_makeDatacard(ws_sig_path=ws_path_signal, output_card_name = "Datacard_" + mass +"_"+year+"_" + cat, channel=mass,process='gghh'+ final_state) 
