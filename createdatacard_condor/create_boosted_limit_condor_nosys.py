...
parser.add_argument('--input_dir',type=str,default="/eos/cms/store/group/phys_b2g/shsong/flashggws/cat12/", help='input_dir')
parser.add_argument('--input_dirname',type=str,default="2018_nosignal_constrain", help='input_dirname')
...
if year == "2017" or year == "2018":
    ...
    yield_cat = "16boosted"
input_path_name = input_dir + input_dirname + "/" + mass + "/"
# ----------------------------  trees2WS part ---------------------------- #
def run_Tree2WS_sig(inputpath_name, inputfile_name, ws_path, output_sig_root_name, process):
    ...
    command = " python trees2ws.py --inputConfig config_simple.py --inputTreeFile " + inputpath_name+inputfile_name + " --inputMass 125  --productionMode " + process_prefix + " --year 2018 "
...
def run_yields(ws_sig_path,ws_bkg_path, inputpath_name, process):
    os.chdir("./Datacard")
    # run Runyields
    command = "python RunYields.py --inputWSDirMap "+year+"=" +inputpath_name+ process + "_" + ws_sig_path+" --sigModelWSDir " + inputpath_name+ process + "_" + ws_sig_path+" --bkgModelWSDir " + inputpath_name+  ws_bkg_path + " --cats auto --procs auto --batch local --ext "+ ws_sig_path
...
def run_makeDatacard(ws_sig_path,output_card_name,channel,process):
    os.chdir("./Datacard")
    # run RunmakeDatacard
    command = "python makeDatacard.py --years "+year+" --prune --ext " + ws_sig_path + " --output "+ output_card_name
...
for cat in cat_list:
    ...
    # # ------------------------------------- signal fit -------------------------------------
    input_file_name_signal = "CombineFHSL_"+ mass +"_"+year+"_"+cat+".root"
