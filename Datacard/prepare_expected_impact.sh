...
for MX in "${MX_VALUES[@]}"; do
  for MY in "${MY_VALUES[@]}"; do
      if [ $MX -gt $((MY+100)) ]; then
          ...
          mkdir YH_cat34_impact/MX${MX}_MY${MY}/
          cp $FILENAME YH_cat34_impact/MX${MX}_MY${MY}/
          cd YH_cat34_impact/MX${MX}_MY${MY}/
          text2workspace.py $FILENAME -m 125
          combineTool.py -M Impacts -d MX${MX}_MY${MY}_run2_cat34.root -m 125 --doInitialFit --robustFit 1 --cminDefaultMinimizerStrategy 0 --X-rtd MINIMIZER_freezeDisassociatedParams --X-rtd MINIMIZER_multiMin_hideConstants  --setParameter MH=125.38 --freezeParameters MH --rMin -5 --rMax 5  -t -1 --expectSignal 1 --freezeParameters rgx{.*},MH
          combineTool.py -M Impacts -d MX${MX}_MY${MY}_run2_cat34.root -m 125 --robustFit 1 --doFits --cminDefaultMinimizerStrategy 0 --X-rtd MINIMIZER_freezeDisassociatedParams --X-rtd MINIMIZER_multiMin_hideConstants --setParameter MH=125.38 --freezeParameters MH --job-mode condor --sub-opts='+JobFlavour="microcentury"' --task-name mytask --rMin -5 --rMax 5 -t -1 --expectSignal 1 
          cd -
      fi
    done
done
...
for MX in "${MX_VALUES[@]}"; do
  for MY in "${MY_VALUES[@]}"; do
      if [ $MX -gt $((MY+100)) ]; then
          ...
          plotImpacts.py -i MX${MX}_MY${MY}_run2_cat34.json -o MX${MX}_MY${MY}_run2_cat34 
          cd -
      fi
    done
done
