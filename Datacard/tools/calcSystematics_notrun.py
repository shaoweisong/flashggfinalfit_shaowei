...
def getValueFromJson(row,uncertainties,sname):
  ...
  p = re.sub("_2016pre_%s"%decayMode,"",row['proc'])
...
def factoryType(d,s):
  ...
  for ir, r in d[d['type']=='sig'].iterrows():
    ...
    elif "PNXbb_sf_" in s['name']: return "s_w"
    else:
      ws.Delete()
      f.Close()
  ...
  for s,f in _systFactoryTypes.iteritems():
    ...
    if f == "a_h": continue
      ...
      if( "%sUp01sigma"%s not in _nominalDataContents )|( "%sDown01sigma"%s not in _nominalDataContents ):
        if "PNXbb_sf" in s and "PNXbb_sf__Up01sigma" not in s:
          systYields["%s_up"%s] = 1
          systYields["%s_down"%s] = 1
        else:
          systToSkip.append(s)
	...
	if not ignoreWarnings: sys.exit(1) 
      ...
      if s not in _nominalDataContents and "PNXbb_sf" not in s:
	systToSkip.append(s)
	print " --> [%s] Weight in nominal RooDataSet for systematic (%s) does not exist for (%s,%s). %s"%(errMessage,s,proc,year,errString)
	if not ignoreWarnings: sys.exit(1)
...
def compareYield(row,factoryType,sname,mode='default',mname=None):
  ...
  if( mode == 'default' )|( mode == 'ishape' ):
    ...
    elif "PNXbb_sf" in sname and "PNXbb_sfs_down_yield" not in row.keys():
      return [1.,1.]
