...
for cidx in range(len(cats)):
  ...
  if opt.doResonantBackground:
    ...
    h_bpdf = {'pdfNBins':sbpdf.createHistogram("h_hb_pdfNBins_%s"%c,_xvar,ROOT.RooFit.Binning(opt.pdfNBins,xvar.getMin(),xvar.getMax())),
    ...
    h_continuumpdf = {'pdfNBins':sbpdf.createHistogram("h_hcont_pdfNBins_%s"%c,_xvar,ROOT.RooFit.Binning(opt.pdfNBins,xvar.getMin(),xvar.getMax())),
               'nBins':sbpdf.createHistogram("h_hcont_nBins_%s"%c,_xvar,ROOT.RooFit.Binning(opt.nBins,xvar.getMin(),xvar.getMax()))
    }
