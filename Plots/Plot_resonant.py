...
def createDataCardsThetaB(labels, values):
    ...
    for label, theta_B in zip(labels, values):
        ...
        for line in datacard_lines:
            datacard.write(line+"\n")
        theta_B_formatted = ("%.3f" % (1+theta_B/100)
                             ).rstrip('0').rstrip('.')  # format
...
def executeDataCards(labels):
    ...
    for label in labels:
        ...
        combine_command = "combine -M AsymptoticLimits -m 125 -n %s %s" % (
            label, file_name)
        print ""
...
def plotUpperLimits(labels, values, resultType):
    ...
    for i in range(N):
        ...
        HHWWgg_lumiRescaleFactor = 1.
        if(resultType == "WWgg"):
            HHWWgg_factor = HHWWgg_WWgg_factor*HHWWgg_lumiRescaleFactor
        elif(resultType == "HH"):
            HHWWgg_factor = HHWWgg_lumiRescaleFactor
        ...
        print("limits",labels[i],limit[4],limit[3],limit[2],limit[1],limit[0])
    ...
    if(args.unit == "pb"):
        ...
        if(resultType == "WWgg"):
            frame.GetYaxis().SetTitle(
                "95% CL limits on #sigma(gg#rightarrow X)#times B(X#rightarrow HH#rightarrow bb#gamma#gamma) [fb]")
        elif(resultType == "HH"):
            frame.GetYaxis().SetTitle(
...
#    legend.AddEntry(green, "AsymptoticLimits CL_{s} #pm 2 std. deviation",'f')
    ...
    # args.lumiRescale
    c.SaveAs("%s%s_UpperLimit.pdf" % (outFile,args.resultType))
    c.SaveAs("%s%s_UpperLimit.png" % (outFile,args.resultType))
    c.SaveAs("%s%s_UpperLimit.C" % (outFile,args.resultType))
    c.Close()
...
def plotNonResUpperLimits(campaign, labels, resultType, plotLabels):
    ...
    for i in range(N):
        ...
        file_name = "./FL_result/higgsCombineFL" + \
            labels[i]+".AsymptoticLimits.mH125.root"
...
#    frame.GetYaxis().SetTitle("95% upper limit on #sigma #times BR / (#sigma #times BR)_{SM}")
    ...
    label.SetLineWidth(2)
    label.DrawLatex(0.16, 0.87,"HH #rightarrow WW#gamma#gamma #rightarrow l#nul#nu#gamma#gamma")
...
def main():
    ...
    if args.Ratio:
        ...
        ol = '/afs/cern.ch/user/c/chuw/chuw/HHWWgg/CMSSW_10_2_13/src/flashggFinalFit/FL_EFT_limits/'
